...
(function () {
  ...
  window.drawPieChart = function (data, options) {
    var W = this.clientWidth,
      H = this.clientHeight,
      y = 0;
    const totalRepos = data.map(d=>d.value).reduce((a,b)=>a+b);
    ...
    if (options.legend) {
      ...
      data.forEach(function (cData) {
        ...
        var percentage = Math.round((cData.value/totalRepos)*100);
        text.innerHTML = `${cData.title} : ${cData.value} (${percentage}%)`;
