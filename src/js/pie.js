...
;(() => {
  ...
  const firstElementFromHtmlString = (string) => {
    const div = document.createElement('div');
    div.innerHTML = string;
    return div.childNodes[0];
  }

  window.drawPieChart = (data, options) => {
    ...
    let centerX = W/2,
      ...
      settings = extend({
        ...
        beforeDraw: () => {  },
        afterDrawed : () => {  },
        onPieMouseenter : (e,data) => {  },
        onPieMouseleave : (e,data) => {  },
        onPieClick : (e,data) => {  },
        legend: false
      }, options),
      animationOptions = {
    ...
    const wrapper = firstElementFromHtmlString('<svg width="' + W + '" height="' + (H + y) + '" viewBox="0 0 ' + W + ' ' + (H + y) + '" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"></svg>');
    ...
    function pathMouseEnter(e){
      const index = this.dataset.order;
    ...
    const drawPieSegments = (animationDecimal) => {
    ...
    const animFrameAmount = (settings.animation)? 1/settings.animationSteps : 1;//if settings.animationSteps is 10, animFrameAmount is 0.1
